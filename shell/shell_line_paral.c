...
int check(char *,int);
void free_list(list **);
void print_argv1(char **,pid_t);
void print_argv2(char **,pid_t);
...
void Father_handler_one(int sig)
{
    ...
    if (sig == SIGINT)
    {
        ...
        for (i = 0; i<am_ch; i++)
        {
            if (arr[i]->pid != -1)
            {
                print_argv1(arr[i]->argv_son,arr[i]->pid);
            }
        }
        return;
    }
    if (sig == SIGTSTP)
    {
        ...
        for (i = 0; i<am_ch; i++)
        {
            if (arr[i]->pid != -1)
            {
                print_argv2(arr[i]->argv_son,arr[i]->pid);
            }
        }
        for (i = 0; i<am_ch; i++)
        {
...
void print_argv1(char ** argv,pid_t pid)
{
    int counter = 0;
...
void print_argv2(char ** argv,pid_t pid)
{
    int counter = 0;
    fprintf(stdout,"::SIGTSTP~pid: %d with argv: ",pid);
    while (argv[counter] != NULL)
    {
        fprintf(stdout,"%s ",argv[counter]);
        counter++;
